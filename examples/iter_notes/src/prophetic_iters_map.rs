...
pub trait Iterator {
    ...
    /// How many items will this iterator eventually return before returning None?  
    /// Returns None if the iterator continues indefinitely or returns Some after returning None.
    #[verifier::prophetic]
    spec fn length(self) -> Option<int>;

    /// Value that was (or will be) returned by the i-th call to `next()`
    #[verifier::prophetic]
...
impl <'a, T> VecIterator<'a, T> {
    pub closed spec fn new(v: &'a Vec<T>) -> Self {
        VecIterator { v, i: 0, j: v.len(), i_g: Ghost(0), j_g: Ghost(v.len() as int) }
    }
...
impl<'a, T> Iterator for VecIterator<'a, T> {
    ...
    closed spec fn length(self) -> Option<int> {
        Some(self.v@.len() as int)
    }
