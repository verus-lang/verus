...
impl<T: Iter> Skip3<T> {
    fn new(iter: T) -> (r: Skip3<T>)
        ...
        ensures
            r.inv(),
            r.iters().len() == r.items().len() + 1,
            r.0.items().len() == iter.items().len() + 3,
            r.0.iters().len() == iter.iters().len() + 3,
            forall|i: int|
                #![trigger r.items()[i]]
                #![trigger r.iters()[i]]
                0 <= i < r.items().len() ==> {
...
impl<T: Iter> Iter for Skip3<T> {
    ...
    spec fn iters(&self) -> Seq<Self> {
        self.0.iters().skip(3).map(|_i, item| Skip3(item))
    }
