...
impl<T: Iter> Skip3<T> {
    fn new(iter: T) -> (r: Skip3<T>)
        ...
        ensures
            r.inv(),
            r.inner.items().len() == iter.items().len() + 3,
            r.start@ == iter.items().len(),
            r.items() == r.inner.items().skip(3 + r.start@),
            iter.reaches(r.inner),
    {
        reveal(reaches_transitive_after_next_if_requested);
        let mut iter = iter;
        let ghost start = iter.items().len();
        let _ = iter.next();
        let _ = iter.next();
        let _ = iter.next();
        Skip3 { inner: iter, start: Ghost(start as int) }
    }
}
...
impl<T: Iter> Iter for Skip3<T> {
    ...
    spec fn items(&self) -> Seq<Option<Self::Item>> {
        self.inner.items().skip(3 + self.start@)
    }

    spec fn inv(&self) -> bool {
        &&& self.inner.inv()
        &&& 0 <= self.start@
        &&& self.start@ + 3 <= self.inner.items().len()
    }

    spec fn reaches(&self, dest: Self) -> bool {
        self.inner.reaches(dest.inner) && self.start == dest.start
    }
...
fn test_loop0(v: &MyVec<u8>) {
    ...
    loop
        ...
        invariant
            i0.vec == v, // should be a for loop auto-invariant
        ...
        v@.len() >= 6,
{
    ...
    loop
        invariant_except_break
            0 <= iter.items().len() <= v@.len() - 6, // should be a for loop auto-invariant
