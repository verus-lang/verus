...
pub fn test() {
    ...
    let lperm: Tracked<LoadPerm> = Tracked(LoadPerm { var: orig_var });
    let (res, Tracked(load_var)) = ctr.load(lperm);

    assert(load_var@ == 42);
    assert(res == 42);

    let iperm: Tracked<IncPerm> = Tracked(IncPerm { var: load_var });
    let (res, Tracked(inc_var)) = ctr.inc(iperm);

    assert(inc_var@ == res.wrapping_add(1));
}
