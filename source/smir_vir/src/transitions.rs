...
use vir::ast_util::{conjoin, mk_call, mk_var, mk_and, mk_let_in, mk_implies, mk_or, mk_ife};
use vir::ast::{
...
fn check_updates_refer_to_valid_fields(fields: &Vec<Field<Ident, Typ>>, ts: &TransitionStmt<Span, Ident, Expr>) -> Result<(), VirErr> {
    match ts {
...
fn disjoint_union(h1: &Vec<(Ident, Span)>, h2: &Vec<(Ident, Span)>) -> Result<Vec<(Ident, Span)>, VirErr> {
...
fn check_has_all_fields(sp: &Span, h: &Vec<(Ident, Span)>, fields: &Vec<Field<Ident, Typ>>) -> Result<(), VirErr> {
    for field in fields {
        if !update_set_contains(h, &field.ident) {
            return Err(error(format!("onitialization does not initialize field {}", *field.ident), &sp));
        }
    }
    Ok(())
}
    ...
    match ts {
        ...
        TransitionStmt::If(sp, _, thn, els) => {
            ...
            if !update_sets_eq(&h1, &h2) {
                let mut e = error("for initialization, both branches of if-statement must update the same fields", &sp);
                for (ident, sp) in &h1 {
                    if !update_set_contains(&h2, ident) {
                        e = e.primary_label(&sp, format!("only the first branch updates {}", *ident));
                    }
                }
                for (ident, sp) in &h2 {
                    if !update_set_contains(&h1, ident) {
                        e = e.primary_label(&sp, format!("only the second branch updates {}", *ident));
                    }
                }
...
pub fn check_transitions(sm: &SM<Span, Ident, Ident, Expr, Typ>, fun_map: &HashMap<Ident, Function>) -> Result<(), VirErr> {
...
pub fn expr_is_enabled(
    ...
    match ts {
        ...
        TransitionStmt::Let(_, _, _) => { None /* see previous case */ }
