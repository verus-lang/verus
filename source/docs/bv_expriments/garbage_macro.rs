...
#[proof]
#[verifier(external_body)]
fn get_bit_primitive(bv:u32, loc:u32) {
    ensures(get_bit!(bv,loc) == get_bit(bv,loc));
}

#[proof]
#[verifier(external_body)]
fn set_bit_primitive(bv:u32, loc:u32, bit:bool) {
    requires(loc < 32);
    ensures(set_bit!(bv,loc,bit)== set_bit(bv,loc,bit));
}
...
fn set_two_bit_exec(bv:u32, low_loc:u32, high:bool, low:bool) -> u32 {
    ...
    assert_bit_vector(
        ...
        == low)); 
    get_bit_primitive(result , low_loc);
    ...
    assert_bit_vector(
        ...
        == high));
    get_bit_primitive(result , low_loc+1);
    ...
    forall(|loc2:u32| { 
        ...
        get_bit_primitive(bv, loc2);
        get_bit_primitive(result, loc2);
    });
    result
}
...
fn get_color(bv:u32, index:u32) -> Color {
    ...
    get_bit_primitive(bv, 2*index);
    get_bit_primitive(bv, 2*index+1);
