...
### Overflow and `exec_spec_trusted!`

When using `exec_spec_trusted!`, Verus does not generate proofs that the executable code produced by the macro does not overflow. This is one difference between `exec_spec_trusted!` and from `exec_spec!`. Instead, the code generated by Verus will have a runtime error if overflow occurs.
