# Producing executable code from spec functions with the `exec_spec!` and `exec_spec_trusted!` macros
...
The `exec_spec_trusted!` macro also automatically compiles spec code to exec code,
but without producing specifications on the exec code or proofs of equivalence. 
...
## Trusted `vstd` functions within by the `exec_spec!` and `exec_spec_trusted!` macros

To enable greater expressivity, the `exec_spec!` and `exec_spec_trusted!` macros support the use of
...
For many `vstd` functions, however, the translation from spec code to the exec equivalent is trusted. 
Each case is marked clearly below. We hope to reduce the number of trusted translations as Verus expands
...
Here is the same example as above, but with the `exec_spec_trusted!` macro:
```rust
{{#include ../../../../examples/guide/exec_spec_trusted.rs:example}}
```

The same exec items are generated as with the `exec_spec!` macro, 
...
The `exec_spec_trusted!` macro does not generate Verus proofs for the absence of arithmetic overflow, the satisfaction of function preconditions, or termination within the compiled executable code. This differs from `exec_spec!`. Instead, the code generated by `exec_spec_trusted!` will throw a runtime error if a precondition is violated (e.g. unwrapping a `None`) or if overflow occurs.

### Fully qualified paths

The `exec_spec!` and `exec_spec_trusted!` macros do not currently support fully qualified paths, such as `vstd::multiset::Multiset`. To use types from another module, you should import the module at the top of the file and use the unqualified path (in this previous case, this would be `Multiset`). Using the fully qualified path will result in a type error.
