...
> **Note:** In spec mode, `==` is mathematical equality — it is not the same as `==` in exec code,
...
In some cases, Verus may accept `==` as well-typed when both sides have the same
[mathematical interpretation](./spec-types.md). For example:
...
The `==` operator returns `true` iff the two sides are equivalent in their [mathematical interpretation](./spec-types.md). The `!=` operator is the negation.
