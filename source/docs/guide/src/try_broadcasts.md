...
Due to technical limitations, `try_broadcasts` only takes into account lemmas that
are either are defined in or referenced by the current crate. For example,
absent any other references to `vstd::Seq::lemma_flatten_push`, `try_broadcasts`
fails to find a proof in the following example:

```rust
{{#include ../../../../examples/guide/try_broadcasts.rs:requires_vstd_lemma}}
```

However, the `vstd::Seq::lemma_flatten_push` would be sufficient to complete the
proof. When referencing this lemma anywhere in the current project, `try_broadcasts`
is able to complete the proof. Adding the following `broadcast group` results in
a successful proof search:

```rust
{{#include ../../../../examples/guide/try_broadcasts.rs:vstd_lemma_reference}}
```

```
note: try_broadcasts found proof with 1 lemma:
      broadcast use vstd::seq::Seq::lemma_flatten_push;
  --> ../examples/guide/try_broadcasts.rs:51:7
   |
...
contains it, is sufficient to make this lemma visible to `try_broadcasts`. It can be
helpful to include large lemma groups such as
`vstd::seq_lib::group_seq_properties` and `vstd::set_lib::group_set_properties`
in `broadcast group try_broadcasts_hints` to ensure that a lot of common
properties are available to try_broadcasts. Similarly, for library code intended
