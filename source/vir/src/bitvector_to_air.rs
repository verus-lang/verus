use crate::ast::{
    ...
    IntegerTypeBoundKind, SpannedTyped, Typ, TypX, UnaryOp, UnaryOpr, VirErr,
};
use crate::ast_util::{
...
use air::ast_util::{bool_typ, mk_and, mk_implies, mk_ite, mk_or, mk_xor, str_typ, string_var};
...
fn bv_maybe_split(ctx: &Ctx, exp: &Exp) -> Result<Vec<BvSpecialized>, VirErr> {
    ...
    let mut state = State { arch: ctx.global.arch, decls: vec![], scope_map: ScopeMap::new() };
    ...
    if state.arch != ctx.global.arch {
        ...
        let mut state =
            State { arch: ArchWordBits::Exactly(64), decls: vec![], scope_map: ScopeMap::new() };
    ...
    match &exp.x {
        ...
        ExpX::Binary(BinaryOp::Bitwise(BitwiseOp::Shr(_), _), lhs, rhs) => {
            ...
            if rhs_extend != Extend::Zero {
                return Err(error(
                    &exp.span,
                    format!("not supported: bit-shift with possibly negative shift"),
                ));
            }
        ...
        match int_range {
            ...
            Some(ir) => {
                ...
                if ir_w <= lossless_w {
                    ...
                    // The clip-width is actually *larger* than the lossless_w
                    // In this case, we perform the lossless arith operation, then
                    // do the clip.
                    ...
                    // We still need to clip (though since lossless_w < ir_w
                    // this is only nontrivial when clipping from signed to unsigned)
