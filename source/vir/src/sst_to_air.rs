        ...
        StmX::Return { base_error, ret_exp, inside_body, assert_id } => {
            ...
            let mut stmts = if skip {
                ...
                let mut stmts = if let Some(dest_id) = state.post_condition_info.dest.clone() {
                    ...
                    let mut stmts = stm_to_stmts(ctx, state, &assume_var(&stm.span, &dest_id, ret_exp))?;
                    // let mut stmts = if is_async_fn == false {
                    //     stm_to_stmts(ctx, state, &assume_var(&stm.span, &dest_id, ret_exp))?
                    // } else {
                    //     async_fn_return_to_stmts(
                    //         ctx,
                    //         state,
                    //         expr_ctxt,
                    //         &mut ret_typ_op,
                    //         &async_body_return_type,
                    //         ret_exp,
                    //         &stm,
                    //         dest_id,
                    //     )?
...
fn async_fn_return_to_stmts(
    ...
    let async_body_return_type =
        async_body_return_type.as_ref().expect("async function has no return type");
    let call = ExpX::Call(
        CallFun::Fun(
