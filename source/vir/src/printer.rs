...
pub struct Printer {
    // 0: Log all but span, 1: Log all. Default is 0.
    log_config: u32,
}

impl Printer {
    pub fn new(log_config: u32) -> Printer {
        Printer { log_config }
    }
    fn spanned_node(&self, node: Node, span: &Span) -> Node {
        if self.log_config >= LOG_ALL {
    ...
    fn pattern_to_node(&self, pattern: &Pattern) -> Node {
        let node = match &pattern.x {
            ...
            PatternX::Constructor(path, ident, binders) => {
                nodes!(constructor {path_to_node(path)} {str_to_node(ident)} {binders_node(binders, &|x| self.pattern_to_node(x))})
            }
        };
    ...
    fn expr_to_node(&self, expr: &Expr) -> Node {
        let node = match &expr.x {
            ...
            ExprX::Ctor(path, ident, binders, expr) => {
                let mut nodes = nodes_vec!(ctor {path_to_node(path)} {str_to_node(ident)} {binders_node(binders, &|x| self.expr_to_node(x))});
                if let Some(e) = expr {
            ...
            ExprX::Quant(quant, binders, expr) => {
                nodes!({str_to_node(&format!("{:?}", quant.quant).to_lowercase())} {binders_node(binders, &typ_to_node)} {self.expr_to_node(expr)})
            }
            ExprX::Closure(binders, expr) => {
                nodes!(closure {binders_node(binders, &typ_to_node)} {self.expr_to_node(expr)})
            }
            ExprX::Choose { params, cond, body } => {
                nodes!(choose {binders_node(params, &typ_to_node)} {self.expr_to_node(cond)} {self.expr_to_node(body)})
            }
            ...
            ExprX::Forall { vars, require, ensure, proof } => {
                nodes!(forall {binders_node(vars, &typ_to_node)} {str_to_node(":require")} {self.expr_to_node(require)} {str_to_node(":ensure")} {self.expr_to_node(ensure)} {str_to_node(":proof")} {self.expr_to_node(proof)})
            }
            ...
            ExprX::OpenInvariant(e1, binder, e2, atomicity) => {
                nodes!(openinvariant {self.expr_to_node(e1)} {binder_node(binder, &typ_to_node)} {self.expr_to_node(e2)} {atomicity_to_node(*atomicity)})
            }
            ExprX::Return(expr) => {
        ...
        for datatype in datatypes.iter() {
            writeln!(
                ...
                nw.node_to_string_indent(
                    &" ".to_string(),
                    &self.spanned_node(datatype_to_node(&datatype.x), &datatype.span)
                )
            )
