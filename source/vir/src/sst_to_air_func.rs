...
pub(crate) fn func_bind_trig(
    ...
    Ok(Arc::new(BindX::Quant(Quant::Forall, Arc::new(binders), triggers, qid)))
}
...
pub(crate) fn func_def_typs_args(ctx: &Ctx, typ_args: &Typs, params: &Pars) -> Vec<Expr> {
...
pub(crate) fn func_def_args(ctx: &Ctx, typ_params: &Idents, params: &Pars) -> Vec<Expr> {
    ...
    func_def_typs_args(ctx, &typ_args, params)
}
...
fn func_def_quant(
    ...
    let f_args = func_def_typs_args(ctx, typ_args, params);
    ...
    Ok(mk_bind_expr(
        &func_bind(ctx, name.to_string(), typ_params, params, &f_app, false)?,
        &f_imply,
    ))
}
    ...
    if specs.len() + typing_invs.len() > 0 {
        ...
        match inherit_from {
            ...
            Some((name, trait_typ_args)) => {
                let args = func_def_typs_args(ctx, &trait_typ_args, params);
...
pub fn func_name_to_air(
    ...
    let declare_rec = |commands: &mut Vec<Command>| {
        ...
        if function.x.has.has_body {
            if function.x.has.is_recursive {
                ...
                let mut rec_typs =
                    vec_map_result(&*function.x.pars, |param| typ_to_air(ctx, &param.x.typ))?;
    ...
    if function.x.mode == Mode::Spec {
        ...
        let mut all_typs = vec_map_result(&function.x.pars, |param| typ_to_air(ctx, &param.x.typ))?;
        for _ in function.x.typ_params.iter() {
            for x in crate::def::types().iter().rev() {
                all_typs.insert(0, str_typ(x));
            }
...
pub fn func_decl_to_air(ctx: &mut Ctx, function: &FunctionSst) -> Result<Commands, VirErr> {
    ...
    let req_typs: Arc<Vec<_>> =
        Arc::new(vec_map_result(&*function.x.pars, |param| typ_to_air(ctx, &param.x.typ))?);
    ...
    let mut ens_typs: Vec<_> = function
        ...
        .pars
        .iter()
        .map(|param| Ok::<_, VirErr>((typ_to_air(ctx, &param.x.typ)?, param.x.is_mut)))
        .flat_map(|res| match res {
            Ok((air_typ, true)) => vec![Ok(air_typ.clone()), Ok(air_typ)],
            Ok((air_typ, false)) => vec![Ok(air_typ)],
            Err(e) => vec![Err(e)],
        })
        .collect::<Result<_, _>>()?;
