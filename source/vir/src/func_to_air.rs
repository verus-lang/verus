...
use air::ast_util::{ident_binder, ident_var, str_apply, string_apply};
use std::rc::Rc;
...
fn func_body_to_air(
    ...
    let binders = Rc::new(vec_map(&function.x.params, |param| {
...
pub fn func_decl_to_air(ctx: &Ctx, function: &Function) -> Result<Commands, VirErr> {
    match (function.x.mode, function.x.ret.as_ref()) {
        (Mode::Spec, Some(ret)) => {
            ...
            let typs = vec_map(&function.x.params, |param| typ_to_air(&param.x.typ));
