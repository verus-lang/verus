use crate::{Crate, Indent, Toolchain, ToolchainList, format_manifest};
use indoc::indoc;
...
fn parse_print_toolchain_rolling_release() {
    ...
    // round-trip
    let observed_toml = format_manifest(&observed).expect("format manifest");
    assert_eq!(toml_str, observed_toml);
}
