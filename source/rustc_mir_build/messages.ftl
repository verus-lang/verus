...
mir_build_verus_const_param_in_pattern = const parameters cannot be referenced in patterns

mir_build_verus_const_pattern_depends_on_generic_parameter =
...
mir_build_verus_initializing_type_with_target_feature_requires_unsafe =
...
mir_build_verus_initializing_type_with_target_feature_requires_unsafe_unsafe_op_in_unsafe_fn_allowed =
...
mir_build_verus_invalid_pattern = `{$non_sm_ty}` cannot be used in patterns
...
mir_build_verus_more_information = for more information, visit https://doc.rust-lang.org/book/ch18-02-refutability.html
...
mir_build_verus_non_partial_eq_match =
    to use a constant of type `{$non_peq_ty}` in a pattern, the type must implement `PartialEq`

mir_build_verus_pattern_not_covered = refutable pattern in {$origin}
    .pattern_ty = the matched value is of type `{$pattern_ty}`

mir_build_verus_pointer_pattern = function pointers and raw pointers not derived from integers in patterns behave unpredictably and should not be relied upon. See https://github.com/rust-lang/rust/issues/70861 for details.
...
mir_build_verus_rust_2024_incompatible_pat = the semantics of this pattern will change in edition 2024

mir_build_verus_rustc_box_attribute_error = `#[rustc_box]` attribute used incorrectly
...
mir_build_verus_type_not_structural =
...
mir_build_verus_type_not_structural_tip = the traits must be derived, manual `impl`s are not sufficient

mir_build_verus_unconditional_recursion = function cannot return without recursing
    .label = cannot return without recursing
    .help = a `loop` may express intention better if this is on purpose

mir_build_verus_unconditional_recursion_call_site_label = recursive call site
...
mir_build_verus_unsafe_op_in_unsafe_fn_initializing_type_with_target_feature_requires_unsafe =
    initializing type with `target_feature` attr is unsafe and requires unsafe block
    .note = this struct can only be constructed if the corresponding `target_feature`s are available
    .label = initializing type with `target_feature` attr
