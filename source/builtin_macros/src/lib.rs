...
#[cfg(verus_keep_ghost)]
pub(crate) fn new_ghost_code() -> bool {
    static CFG_GHOST: OnceLock<bool> = OnceLock::new();
    *CFG_GHOST.get_or_init(|| {
        let ts: proc_macro::TokenStream = quote::quote! { ::core::cfg!(verus_new_ghost_code) }.into();
        let bool_ts = match ts.expand_expr() {
            Ok(name) => name.to_string(),
            _ => {
                panic!("new_ghost_code call failed")
            }
        };
        match bool_ts.as_str() {
            "true" => true,
