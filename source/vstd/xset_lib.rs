...
broadcast use super::xset::group_set_axioms;
...
impl<A> Set<A, true> {
    ...
    {
        let x: Set<A> = self;
        // need to change this proof, or prove a statement about filters in inf space
//         lemma_len_intersect(x, ISet::<A>::new(f));
    }
}
...
pub broadcast proof fn lemma_set_disjoint<A, const Finite: bool, const Finite2: bool>(a: Set<A, Finite>, b: Set<A, Finite2>)
    ensures
        ...
        a.disjoint(b) ==> ((a + b).difference(a) =~= b.to_infinite() && (a + b).difference(b) =~= a.to_infinite()),
{
}
...
pub proof fn lemma_set_properties<A, const Finite1: bool, const Finite2: bool>()
    ensures
        ...
        forall|a: Set<A, Finite1>, b: Set<A, Finite2>|
            ...
            a.disjoint(b) ==> ((a + b).difference(a) =~= b.to_infinite() && (a + b).difference(b) =~= a.to_infinite()),  //from lemma_set_disjoint
...
/// ```
#[macro_export]
macro_rules! xassert_sets_equal {
    [$($tail:tt)*] => {
        ::builtin_macros::verus_proof_macro_exprs!($crate::vstd::xset_lib::xassert_sets_equal_internal!($($tail)*))
    };
}

#[macro_export]
#[doc(hidden)]
macro_rules! xassert_sets_equal_internal {
    (::builtin::spec_eq($s1:expr, $s2:expr)) => {
        $crate::vstd::xset_lib::xassert_sets_equal_internal!($s1, $s2)
    };
    (::builtin::spec_eq($s1:expr, $s2:expr), $elem:ident $( : $t:ty )? => $bblock:block) => {
        $crate::vstd::xset_lib::xassert_sets_equal_internal!($s1, $s2, $elem $( : $t )? => $bblock)
    };
    ...
    ($s1:expr, $s2:expr $(,)?) => {
        $crate::vstd::xset_lib::xassert_sets_equal_internal!($s1, $s2, elem => { })
    };
    ($s1:expr, $s2:expr, $elem:ident $( : $t:ty )? => $bblock:block) => {
        #[verifier::spec] let s1 = $crate::vstd::xset_lib::check_argument_is_set($s1);
        #[verifier::spec] let s2 = $crate::vstd::xset_lib::check_argument_is_set($s2);
...
pub use xassert_sets_equal_internal;
pub use xassert_sets_equal;
