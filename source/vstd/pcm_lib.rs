...
pub proof fn copy_duplicable_part<P: PCM>(tracked r: &Resource<P>, new_value: P) -> (tracked out: Resource<P>)
    requires
...
pub proof fn redistribute<P: PCM>(tracked r1: &mut Resource<P>, tracked r2: &mut Resource<P>, v1: P, v2: P)
    requires
...
pub proof fn update_and_redistribute<P: PCM>(tracked r1: &mut Resource<P>, tracked r2: &mut Resource<P>, v1: P, v2: P)
    requires
        old(r1).loc() == old(r2).loc(),
...
proof fn aggregate_resources_from_map_starting_at_offset<P: PCM>(
    ...
    requires
        ...
        forall |i| #![trigger old(m).dom().contains(i)] offset <= i < values.len() ==> old(m).dom().contains(i),
        forall |i| #![trigger old(m)[i]] offset <= i < values.len() ==> old(m)[i].loc() == id && old(m)[i].value() == values[i],
    ensures
        ...
        values.len() - offset
{
    ...
    if offset == values.len() - 1 {
        ...
        assert(combine_values(values.skip(offset)) ==
               P::op(values[offset], combine_values(values.skip(offset + 1)))) by {
            assert(values[offset] =~= values.skip(offset)[0]);
            assert(values.skip(offset + 1) =~= values.skip(offset).skip(1));
        }
        assert forall |i| #![trigger m.dom().contains(i)] offset + 1 <= i < values.len() implies
            m.dom().contains(i) && m[i].loc() == id && m[i].value() == values[i] by {
            assert(m.dom().contains(i));
            assert(m[i].loc() == id && m[i].value() == values[i]);
        }
        let tracked most = aggregate_resources_from_map_starting_at_offset(m, id, values, offset + 1);
        assert(most.loc() == id);
...
proof fn store_resources_into_map_starting_at_offset<P: PCM>(
    ...
    requires
        0 <= offset <= values.len(),
        forall |i| #![trigger old(m).dom().contains(i)] 0 <= i < offset ==> old(m).dom().contains(i),
        forall |i| #![trigger old(m)[i]] 0 <= i < offset ==> old(m)[i].loc() == id && old(m)[i].value() == values[i],
        forall |i| #![trigger old(m).dom().contains(i)] offset <= i < values.len() ==> !old(m).dom().contains(i),
        p.loc() == id,
        p.value() == combine_values(values.skip(offset)),
    ensures
        forall |i| #![trigger m.dom().contains(i)] 0 <= i < values.len() ==> m.dom().contains(i),
        forall |i| #![trigger m[i]] 0 <= i < values.len() ==> m[i].loc() == id && m[i].value() == values[i],
    decreases
        values.len() - offset
{
    if offset != values.len() {
        ...
        let tracked (p_first, p_rest) = p.split(values[offset], combine_values(values.skip(offset + 1)));
...
pub proof fn validate_multiple<P: PCM>(
    ...
    requires
        forall |i| #![trigger old(m).dom().contains(i)] 0 <= i < values.len() ==> old(m).dom().contains(i),
        forall |i| #![trigger old(m)[i]] 0 <= i < values.len() ==> old(m)[i].loc() == loc && old(m)[i].value() == values[i],
    ensures
        forall |i| #![trigger m.dom().contains(i)] 0 <= i < values.len() ==> m.dom().contains(i),
        forall |i| #![trigger m[i]] 0 <= i < values.len() ==> m[i].loc() == loc && m[i].value() == values[i],
        ...
        P::op(r1.value(), P::op(r2.value(), P::op(r3.value(), r4.value()))).valid(),
{
    ...
    assert (combine_values(values) ==
            P::op(old(r1).value(), P::op(old(r2).value(), P::op(old(r3).value(), old(r4).value())))) by {
        lemma_pcm_properties::<P>();
...
pub proof fn validate_5<P: PCM>(
    ...
    ensures
        ...
        P::op(r1.value(), P::op(r2.value(), P::op(r3.value(), P::op(r4.value(), r5.value())))).valid(),
{
    lemma_pcm_properties::<P>();
    ...
    assert (combine_values(values) ==
            P::op(old(r1).value(), P::op(old(r2).value(), P::op(old(r3).value(), P::op(old(r4).value(), old(r5).value()))))) by {
        lemma_pcm_properties::<P>();
