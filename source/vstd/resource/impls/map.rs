...
broadcast use {super::super::super::group_vstd_default, super::super::super::map::group_map_lemmas};
...
impl<K, V> GhostMapAuth<K, V> {
    ...
    /// Insert an [`Map`] of values, receiving the [`GhostSubmap`] that asserts ownership over the key
    /// domain inserted.
    ///
    /// ```
            ...
            result@ == m,
    {
        ...
        assert forall|c: MapCarrier<K, V>| #[trigger]
            MapCarrier::op(self_r.value(), c).valid() implies MapCarrier::op(
            full_carrier,
            c,
        ).valid() by {
            assert(full_carrier.frac.owning_map().dom().disjoint(c.frac.dup_map().dom()));
        }
    ...
    /// Create a new [`GhostMapAuth`] from an [`Map`].
