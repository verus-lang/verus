...
pub mod fold {
    ...
    pub open spec fn is_fun_commutative<A, B>(f: spec_fn(A, B) -> B) -> bool {
        forall|a1, a2, b| #[trigger] f(a1, f(a2, b)) == f(a2, f(a1, b))
    }
    ...
    #[verifier(opaque)]
    spec fn fold_graph<A, B>(f: spec_fn(A, B) -> B, z: B, s: Set<A>, y: B, d: nat) -> bool
        decreases d,
    {
        if s === Set::empty() {
            &&& z == y
            ...
            exists|yr, a|
                {
                    ...
                    &&& s.remove(a).finite()
                    &&& s.contains(a)
                    &&& fold_graph(f, z, s.remove(a), yr, sub(d, 1))
                    &&& y == f(a, yr)
                }
        }
    }
    ...
    // Introduction rules
    proof fn lemma_fold_graph_empty_intro<A, B>(f: spec_fn(A, B) -> B, z: B)
        ensures
            fold_graph(f, z, Set::empty(), z, 0),
    {
        reveal(fold_graph);
    }
    ...
    proof fn lemma_fold_graph_insert_intro<A, B>(
        ...
        requires
            fold_graph(f, z, s, y, d),
            !s.contains(a),
        ensures
            fold_graph(f, z, s.insert(a), f(a, y), d + 1),
    {
    ...
    // Elimination rules
    proof fn lemma_fold_graph_empty_elim<A, B>(f: spec_fn(A, B) -> B, z: B, y: B, d: nat)
        requires
            fold_graph(f, z, Set::empty(), y, d),
        ensures
            z == y,
            d == 0,
    {
    ...
    proof fn lemma_fold_graph_insert_elim<A, B>(
        ...
        requires
            ...
            fold_graph(f, z, s.insert(a), y, d),
            !s.contains(a),
        ensures
            d > 0,
            exists|yp| y == f(a, yp) && #[trigger] fold_graph(f, z, s, yp, sub(d, 1)),
    {
        reveal(fold_graph);
        lemma_fold_graph_insert_elim_aux(f, z, s.insert(a), y, d, a);
        assert(s.insert(a).remove(a) =~= s);
        let yp = choose|yp| y == f(a, yp) && #[trigger] fold_graph(f, z, s, yp, sub(d, 1));
    }
    ...
    proof fn lemma_fold_graph_insert_elim_aux<A, B>(
        ...
        requires
            ...
            s.contains(a),
        ensures
            exists|yp| y == f(a, yp) && #[trigger] fold_graph(f, z, s.remove(a), yp, sub(d, 1)),
        decreases d,
    {
        broadcast use group_set_axioms_early;
        ...
        let (yr, aa): (B, A) = choose|yr, aa|
            #![all_triggers]
            {
                ...
                &&& s.remove(aa).finite()
                &&& s.contains(aa)
                &&& fold_graph(f, z, s.remove(aa), yr, sub(d, 1))
                &&& y == f(aa, yr)
            };
        ...
        if s.remove(aa) === Set::empty() {
            ...
            if a == aa {
                ...
                lemma_fold_graph_insert_elim_aux(f, z, s.remove(aa), yr, sub(d, 1), a);
                let yrp = choose|yrp|
                    yr == f(a, yrp) && #[trigger] fold_graph(
                        f,
                        z,
                ...
                assert(fold_graph(f, z, s.remove(aa).insert(aa).remove(a), f(aa, yrp), sub(d, 1)))
                    by {
    ...
    proof fn lemma_fold_graph_induct<A, B>(
        ...
        requires
            ...
            pred(Set::empty(), z, 0),
            forall|a, s, y, d|
                pred(s, y, d) && !s.contains(a) && #[trigger] fold_graph(f, z, s, y, d) ==> pred(
                    #[trigger] s.insert(a),
                    f(a, y),
                    d + 1,
                ),
        ...
        decreases d,
    {
        ...
        reveal(fold_graph);
        if s === Set::empty() {
            lemma_fold_graph_empty_elim(f, z, y, d);
        } else {
            let a = s.choose();
            lemma_fold_graph_insert_elim(f, z, s.remove(a), y, d, a);
            let yp = choose|yp|
                y == f(a, yp) && #[trigger] fold_graph(f, z, s.remove(a), yp, sub(d, 1));
            lemma_fold_graph_induct(f, z, s.remove(a), yp, sub(d, 1), pred);
        }
    }

    impl<A> Set<A> {
        pub closed spec fn fold<B>(self, f: spec_fn(A, B) -> B, z: B) -> B
            recommends
                self.finite(),
                is_fun_commutative(f),
        {
            let (y, d): (B, nat) = choose|y, d| fold_graph(f, z, self, y, d);
            y
        }
    }

    proof fn lemma_fold_graph_finite<A, B>(f: spec_fn(A, B) -> B, z: B, s: Set<A>, y: B, d: nat)
        requires
            is_fun_commutative(f),
            fold_graph(f, z, s, y, d),
        ensures
            s.finite(),
    {
        ...
        lemma_fold_graph_induct(f, z, s, y, d, pred);
    }
    ...
    proof fn lemma_fold_graph_deterministic<A, B>(
        ...
        requires
            is_fun_commutative(f),
            fold_graph(f, z, s, y1, d1),
            fold_graph(f, z, s, y2, d2),
        ensures
            y1 == y2,
            d1 == d2,
    {
        let pred = |s: Set<A>, y1: B, d1: nat|
            forall|y2, d2| fold_graph(f, z, s, y2, d2) ==> y1 == y2 && d2 == d1;
        // Base case
        assert(pred(Set::empty(), z, 0)) by {
            assert forall|y2, d2| fold_graph(f, z, Set::empty(), y2, d2) implies z == y2 && d2
                == 0 by {
                lemma_fold_graph_empty_elim(f, z, y2, d2);
            };
        };
        // Step case
        assert forall|a, s, y1, d1|
            pred(s, y1, d1) && !s.contains(a) && #[trigger] fold_graph(
                ...
                y1,
                d1,
            ) implies pred(#[trigger] s.insert(a), f(a, y1), d1 + 1) by {
            assert forall|y2, d2| fold_graph(f, z, s.insert(a), y2, d2) implies f(a, y1) == y2 && d2
                == d1 + 1 by {
                lemma_fold_graph_insert_elim(f, z, s, y2, d2, a);
            };
        };
        lemma_fold_graph_induct(f, z, s, y2, d2, pred);
    }

    proof fn lemma_fold_is_fold_graph<A, B>(f: spec_fn(A, B) -> B, z: B, s: Set<A>, y: B, d: nat)
        requires
            is_fun_commutative(f),
            fold_graph(f, z, s, y, d),
        ensures
            s.fold(f, z) == y,
    {
        lemma_fold_graph_finite(f, z, s, y, d);
        if s.fold(f, z) != y {
            let (y2, d2) = choose|y2, d2| fold_graph(f, z, s, y2, d2) && y2 != y;
            lemma_fold_graph_deterministic(f, z, s, y2, y, d2, d);
            assert(false);
        }
    }
    ...
    proof fn lemma_fold_graph_exists<A, B>(f: spec_fn(A, B) -> B, z: B, s: Set<A>)
        requires
            s.finite(),
            is_fun_commutative(f),
        ensures
            exists|y, d| fold_graph(f, z, s, y, d),
    {
        let pred = |s| exists|y, d| fold_graph(f, z, s, y, d);
        // Base case
        assert(fold_graph(f, z, Set::empty(), z, 0)) by {
            lemma_fold_graph_empty_intro(f, z);
        };
        // Step case
        assert forall|s, a| pred(s) && s.finite() && !s.contains(a) implies #[trigger] pred(
            s.insert(a),
        ) by {
            let (y, d): (B, nat) = choose|y, d| fold_graph(f, z, s, y, d);
            lemma_fold_graph_insert_intro(f, z, s, y, d, a);
        };
        lemma_finite_set_induct(s, pred);
    }

    pub broadcast proof fn lemma_fold_insert<A, B>(s: Set<A>, f: spec_fn(A, B) -> B, z: B, a: A)
        requires
            s.finite(),
            !s.contains(a),
            is_fun_commutative(f),
        ensures
            #[trigger] s.insert(a).fold(f, z) == f(a, s.fold(f, z)),
    {
        lemma_fold_graph_exists(f, z, s);
        let (y, d): (B, nat) = choose|y, d| fold_graph(f, z, s, y, d);
        lemma_fold_graph_insert_intro(f, z, s, s.fold(f, z), d, a);
        lemma_fold_is_fold_graph(f, z, s.insert(a), f(a, s.fold(f, z)), d + 1);
    }

    pub broadcast proof fn lemma_fold_empty<A, B>(f: spec_fn(A, B) -> B, z: B)
        ensures
            #[trigger] Set::empty().fold(f, z) == z,
    {
        let (y, d): (B, nat) = choose|y, d| fold_graph(f, z, Set::empty(), y, d);
        lemma_fold_graph_empty_intro(f, z);
        lemma_fold_graph_empty_elim(f, z, y, d);
    }
...
{
    fold::lemma_fold_empty(|a: A, b: nat| b + 1, 0);
}
        ...
        }),
{
    if s.contains(a) {
        ...
        fold::lemma_fold_insert(s, |a: A, b: nat| b + 1, 0, a);
    }
}
