...
/// Here we implement the AbstractByteRepresentation for the shadow data of a shared reference, which is its pointer.
///
...
impl<'a, T: ?Sized> AbstractByteRepresentation for ShadowData<&'a T> {
    open spec fn can_be_encoded() -> bool {
        <*const T as AbstractByteRepresentation>::can_be_encoded()
    }

    open spec fn encode(value: Self, bytes: Seq<AbstractByte>) -> bool {
        <*const T as AbstractByteRepresentation>::encode(shared_ref_ptr(value), bytes)
    }

    open spec fn decode(bytes: Seq<AbstractByte>, value: Self) -> bool {
        <*const T as AbstractByteRepresentation>::decode(bytes, shared_ref_ptr(value))
    }
    ...
    proof fn encoding_exists(tracked v: Self) -> (b: Seq<AbstractByte>) {
        ...
        unsigned_int_max_values();
        let prefix = endian_to_bytes(
            EndianNat::<u8>::from_nat(shared_ref_ptr(v)@.addr as nat, size_of::<usize>()),
            Some(shared_ref_ptr(v)@.provenance),
        );

        assert(encoding_exists::<<T as core::ptr::Pointee>::Metadata>(shared_ref_ptr(v)@.metadata));
        let suffix = choose|bytes|
            abs_encode::<<T as core::ptr::Pointee>::Metadata>(
                &(shared_ref_ptr(v)@.metadata),
                bytes,
            );
        let b = prefix.add(suffix);
        assert(b.subrange(0, size_of::<usize>() as int) == prefix);
    ...
    proof fn encoding_invertible(v: Self, b: Seq<AbstractByte>) {
        <*const T as AbstractByteRepresentation>::encoding_invertible(shared_ref_ptr(v), b);
    }
