...
impl<T: DeepView> DeepView for ExecMultiset<T> {
...
impl<'a, T: DeepView + DeepViewClone + std::hash::Hash + std::cmp::Eq> ExecSpecEq<'a> for &'a ExecMultiset<T> where &'a T: ExecSpecEq<'a, Other = &'a T> {
    ...
    #[inline(always)]
    fn exec_eq(this: Self, other: Self::Other) -> bool {
        this.m.len() == other.m.len() && this.m.iter().zip(other.m.iter()).all(
            |(a, b)| <&'a T>::exec_eq(a.0, b.0) && <usize>::exec_eq(*a.1, *b.1)
        )
    }
}
