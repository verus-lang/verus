...
//!
//! - [`GhostSet<T>`] represents authoritative ownership of the entire set;
//! - [`GhostSubset<T>`] represents client ownership of some subset of a subset;
...
impl<T> GhostSetAuth<T> {
    ...
    /// Logical underlying [`Set`]
    pub closed spec fn view(self) -> Set<T> {
        self.map@.dom()
    }

    /// Create a new `GhostSet` from a `Set`.
    ...
    /// }
    /// ```
    pub proof fn delete_points_to(tracked &mut self, tracked p: GhostSingleton<T>)
        requires
            p.id() == old(self).id(),
        ensures
            self.id() == old(self).id(),
...
/// A resource representing ownership of a subset
/// The existence of a `GhostSubset` implies that:
///  - Those values will remain in the set;
...
impl<T> GhostSubset<T> {
    ...
    /// We can separate a single key out of a `GhostSubset`
    pub proof fn split_singlton(tracked &mut self, v: T) -> (tracked result: GhostSingleton<T>)
        requires
            old(self)@.contains(v),
        ensures
            self.id() == old(self).id(),
...
/// A resource representing ownership over a single value in the set
...
impl<T> GhostSingleton<T> {
    ...
    /// Shows that if a `GhostSingleton` and a `GhostSet` are not owning the same value
