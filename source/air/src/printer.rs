...
pub fn singular_query_to_node(vars: Vec<String>, enss: Vec<Expr>, reqs: Vec<Expr>) -> Node {
    let pp = Printer::new(false);
    let mut nodes = Vec::new();
    let mut var_nodes = Vec::new();
    nodes.push(str_to_node("singular-check-valid"));
    let ens = pp.exprs_to_node(&Arc::new(enss));
    let req = pp.exprs_to_node(&Arc::new(reqs));
    for v in vars {
        var_nodes.push(Node::Atom(v));
    }
...
pub(crate) fn expr_to_singular(
    expr: &Expr,
    tmp_idx: &mut u32,
    node_map: &mut HashMap<Node, String>,
) -> String {
    match &**expr {
        ...
        ExprX::Binary(BinaryOp::EuclideanMod, lhs, rhs) => {
            ...
            let t = match value {
                ...
                None => {
                    let tmp_new = format!("tmp_{}", tmp_idx.to_string());
                    *tmp_idx += 1;
                    node_map.insert(key, tmp_new.to_string());
                    tmp_new
                }
            };
...
pub fn singular_printer(vars: Vec<String>, ens_exprs: Vec<Expr>, req_exprs: Vec<Expr>) -> String {
    let mut tmp_count: u32 = 0; // count the number of required tmp vars
    let mut vars2: Vec<String> = vec![];
    let mut node_map: HashMap<Node, String> = HashMap::new(); // for uninterpreted functions and mod translation
