...
//
//  * Every variant optionally has a constructor with a "kind". The ctor_kind() is one of:
//        None, Some(CtorKind::Fn) or Some(CtorKind::Const)
//
//  * For variants that have a constructor, the constructor is given a separate DefId.
//
//  * When a constructor is used as an expression (ExprKind::Path) it will resolve to the
//    the DefId of this constructor (the Res will be of DefKind::Ctor or Res::SelfCtor, i.e.,
//    something with Ctor in its name).
//
//  * When a constructor is used as part of a braces-style constructor (ExprKind::Struct)
//    it resolves to a Variant (for enums) or to a Type (for structs and unions).
