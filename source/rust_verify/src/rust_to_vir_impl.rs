...
use std::sync::Arc;
use vir::ast::{
    Fun, FunX, Function, FunctionKind, Ident, ImplPath, Krate, KrateX, Path, Trait, TraitImpl, Typ,
    Typs, VirErr,
};
...
fn translate_assoc_type<'tcx>(
    ...
    vir.assoc_type_impls.push(ctxt.spanned_new(impl_item_span, assocx));
    Ok(())
}
