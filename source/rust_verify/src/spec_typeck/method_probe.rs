...
pub(crate) fn lookup_method<'tcx>(
    ...
    infcx: InferCtxt<'tcx>,
) -> Result<MethodResult, vir::ast::VirErr> {
    let oc = OuterContext {
        tcx: tcx,
        infcx,
        param_env: param_env,
...
impl<'tcx> OuterContext<'tcx> {
    pub fn lookup_method(
    ...
    ) -> Result<MethodResult, MethodError<'tcx>> {
        let pick =
        ...
        Ok(MethodResult {
            ...
            num_autoderefs: pick.autoderefs,
            autoref_or_ptr_adjustment: pick.autoref_or_ptr_adjustment,
        })
    }
