...
impl State<'_, '_> {
    ...
    pub fn fresh_typ_with_info(&mut self, info: Info) -> Typ {
        ...
        //println!("{:}: {:}", uid, std::backtrace::Backtrace::capture());
    ...
    /// If the given type is an 'unknown integer', concretize it to Int
    pub fn concretize_integer_type_to_int_if_possible(&mut self, t: &Typ) {
        match &**t {
            TypX::UnificationVar(id) => {
                let node = self.unifier.get_class(*id);
                match &self.unifier[node].info {
    ...
    pub fn get_typ_as_concrete_as_possible(&mut self, t: &Typ) -> Result<Typ, VirErr> {
        self.get_typ_with_concrete_head_if_possible(t) // TODO
    }
    ...
    pub fn expect_types_comparable_by_eq(&mut self, t1: &Typ, t2: &Typ) -> Result<(), VirErr> {
        // TODO simplify types here

        let t1c = self.get_typ_as_concrete_as_possible(t1)?;
        let t2c = self.get_typ_as_concrete_as_possible(t2)?;

        let t1c = vir::ast_util::undecorate_typ(&t1c);
        let t2c = vir::ast_util::undecorate_typ(&t2c);
    ...
    fn unify_heads(&mut self, t1: &Typ, t2: &Typ) -> Result<(), UnifyError> {
        match (&**t1, &**t2) {
            ...
            (TypX::ConstInt(a1, _), TypX::ConstInt(a2, _)) => {
                if a1 == a2 {
                    Ok(())
                } else {
                    Err(UnifyError::Error)
