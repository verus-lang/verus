...
use crate::rust_to_vir_base::{mid_ty_to_vir, mid_ty_to_vir_ghost};
use std::sync::Arc;
use rustc_middle::ty::{Generics, GenericParamDefKind, GenericParamDef, VariantDef, GenericArgKind, TraitRef};
...
impl<'a, 'tcx> State<'a, 'tcx> {
    ...
    pub fn trait_ref_lower(&mut self,
        span: Span, trait_ref: TraitRef<'tcx>, unif_map: &HashMap<TyVid, usize>) -> Result<Typs, VirErr>
    {
        let mut vir_typs = vec![];
        for arg in trait_ref.args.iter() {
            match arg.unpack() {
                GenericArgKind::Lifetime(_) => { }
                GenericArgKind::Type(ty) => {
                    let vir_typ = mid_ty_to_vir_ghost(
                        self.tcx,
                        &self.bctx.ctxt.verus_items,
                        self.bctx.fun_id,
                        span,
