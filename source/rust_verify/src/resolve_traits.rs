...
use rustc_middle::ty::{TyCtxt, GenericArgsRef, TypingEnv, TraitRef};
...
pub(crate) fn resolve_trait_item<'tcx>(
    ...
    let pseudo_canonical_inp = rustc_middle::ty::PseudoCanonicalInput { typing_env, value: trait_ref };
            ...
            let resolved_item = match tcx.impl_of_method(item_def_id) {
                Some(impl_def_id_containing_item) => {
                    if impl_def_id_containing_item != impl_def_id {
                        crate::internal_err!(span, "ImplDefIds don't match; this may be caused by the use of the unstable specialization feature");
                    }
                    ResolvedItem::FromImpl(item_def_id, impl_item_args)
                }
                None => {
                    ResolvedItem::FromTrait(item_def_id, args)
                }
            };
