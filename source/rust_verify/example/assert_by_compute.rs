...
#[spec] const empty_seq: Seq<u32> = Seq::empty();
...
fn xor(p: Seq<bool>, q: Seq<bool>) -> Seq<bool> {
    ...
    if p.len() == 0 {
        ...
        xor(p.subrange(0, p.len() as int - 1), q.subrange(0, q.len() as int - 1)).push(p.last() ^ q.last())
    }
}
