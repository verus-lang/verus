...
impl <T> PCM for LogResourceValue<T>
{
    ...
    {
        match (self, other) {
            (Self::PrefixKnowledge{ prefix: prefix1 }, Self::PrefixKnowledge{ prefix: prefix2 }) =>
                if is_prefix(prefix1, prefix2) { other }
                else { if is_prefix(prefix2, prefix1) { self }
                       else { Self::Invalid } },
            (Self::PrefixKnowledge{ prefix }, Self::HalfAuthority{ log }) =>
                if is_prefix(prefix, log) { other } else { Self::Invalid },
            (Self::HalfAuthority{ log }, Self::PrefixKnowledge{ prefix }) =>
                if is_prefix(prefix, log) { self } else { Self::Invalid },
            (Self::PrefixKnowledge{ prefix }, Self::FullAuthority{ log }) =>
                if is_prefix(prefix, log) { other } else { Self::Invalid },
            (Self::FullAuthority{ log }, Self::PrefixKnowledge{ prefix }) =>
                if is_prefix(prefix, log) { self } else { Self::Invalid },
            (Self::HalfAuthority{ log: log1 }, Self::HalfAuthority{ log: log2 }) =>
                if log1 == log2 { Self::FullAuthority{ log: log1 } } else { Self::Invalid },
            (_, _) => Self::Invalid,
        }
    }
