...
impl <T> PCSemigroup for AgreementResourceValue<T>
{
    open spec fn op(self, other: Self) -> Self
    {
        match (self, other) {
            ...
            (AgreementResourceValue::<T>::Chosen { c: c1 }, AgreementResourceValue::<T>::Chosen { c: c2 }) =>
                if c1 == c2 { self } else { AgreementResourceValue::<T>::Invalid { } }
        }
    }
...
impl <T> AgreementResource<T>
{
    ...
    pub proof fn lemma_agreement(tracked self: &mut AgreementResource<T>, tracked other: &AgreementResource<T>)
        requires
            old(self).inv(),
            other.inv(),
