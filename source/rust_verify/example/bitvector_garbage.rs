...
spec fn color_view(high: bool, low: bool) -> Color {
    if high {
        if low { Color::White } else { Color::Gray }
    } else {
        if low { Color::Black } else { Color::Undefined }
    }
}

spec fn bucket_view(bucket: u32) -> Seq<Color> {
    Seq::new(16, |i: int| color_view(get_bit!(bucket, add(mul(i as u32, 2), 1)), get_bit!(bucket, mul(i as u32, 2))))
}

#[verifier(bit_vector)]
proof fn set_two_bit_proof(bv: u32, target: u32, mask: u32, result: u32, low_loc: u32, high: bool, low: bool)
    requires
        low_loc < 31,
        target == (
            if high {
                if low { 3u32 } else { 2u32 }
            } else {
                if low { 1u32 } else { 0u32 }
            }
        ) << low_loc,
        mask == !(3u32 << low_loc),
    ...
    ensures
        ...
        forall|loc2: u32| #![auto]
            loc2 < 32 && loc2 != low_loc && loc2 != add(low_loc, 1) ==>
            get_bit!(result, loc2) == get_bit!(bv, loc2),
{
}
...
fn set_two_bit_exec(bv: u32, low_loc: u32, high: bool, low: bool) -> (ret: u32)
    ...
    ensures
        ...
        forall|loc2: u32| #![auto]
            loc2 < 32 && loc2 != low_loc && loc2 != add(low_loc, 1) ==>
            get_bit!(ret, loc2) == get_bit!(bv, loc2),
{
    let target: u32 =
        (if high {
            if low { 3u32 } else { 2u32 }
