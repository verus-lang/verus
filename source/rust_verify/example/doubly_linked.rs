...
impl<V> DoublyLinkedList<V> {
    ...
    spec fn wf_perm(&self, i: nat) -> bool {
        ...
        &&& match self.perms@[i].0@.value {
            Some(node) => node.prev == self.prev_of(i)
        ...
        // recommends self.well_formed(),
    {
        Seq::<V>::new(self.ptrs@.len(), |i: int| {
            self.perms@[i as nat].0@.value.get_Some_0().value
        })
    }

    pub fn new() -> (s: Self)
        ensures s.well_formed(), s@.len() == 0,
    {
        DoublyLinkedList {
    ...
    fn push_empty_case(&mut self, v: V)
        requires old(self).well_formed(), old(self).ptrs@.len() == 0,
        ensures self.well_formed(), self@ == old(self)@.push(v),
    {
        ...
        ensures self.well_formed(), self@ == old(self)@.push(v),
    {
        match &self.tail {
            ...
            Some(tail_ptr) => {
                ...
                proof {
                    self.perms.borrow_mut().tracked_insert((self.ptrs@.len() - 1) as nat, (tail_perm, tail_dealloc));
                ...
                assert(self.wf_perm((self.ptrs@.len() - 1) as nat));
                assert(forall|i: nat| i < self.ptrs@.len() && old(self).wf_perm(i) ==> self.wf_perm(i));

                assert(self.view().index(self.ptrs@.len() as int - 1) == v);
                assert forall|i: int| 0 <= i && i < self.ptrs@.len() as int - 1 implies old(self)@[i] == self@[i] by {
    ...
    pub fn pop_back(&mut self) -> (v: V)
        requires old(self).well_formed(), old(self)@.len() > 0,
            ...
            v == old(self)@[old(self)@.len() as int - 1],
    {
        ...
        let tracked (last_perm, last_dealloc) = self.perms.borrow_mut().tracked_remove((self.ptrs@.len() - 1) as nat);
        ...
        match &last_node.prev {
            ...
            Some(penult_ptr) => {
                ...
                let tracked (mut penult_perm, penult_dealloc) = self.perms.borrow_mut().tracked_remove((self.ptrs@.len() - 2) as nat);
                ...
                proof {
                    self.perms.borrow_mut().tracked_insert((self.ptrs@.len() - 2) as nat, (penult_perm, penult_dealloc));
                }
            }
        }
        ...
        assert forall|i: int| 0 <= i && i < self@.len()
            implies #[trigger] self@[i] == old(self)@.drop_last()[i] by {
                assert(old(self).wf_perm(i as nat)); // trigger
        }
    ...
    pub fn pop_front(&mut self) -> (v: V)
        requires old(self).well_formed(), old(self).view().len() > 0,
        ensures
            self.well_formed(),
            ...
            v == old(self)@[0],
    {
        ...
        match &first_node.next {
            ...
            Some(second_ptr) => {
                ...
                let tracked (mut second_perm, second_dealloc) = self.perms.borrow_mut().tracked_remove(1);
                ...
                proof {
                    ...
                    assert forall|j: nat| 1 <= j && j < old(self)@.len() implies self.perms@.dom().contains(j) by {
        ...
        proof {
            ...
            assert(forall|i: nat| i < self.view().len() && old(self).wf_perm(i + 1) ==> self.wf_perm(i));

            assert forall|i: int| 0 <= i && i < self@.len() implies
    ...
    pub fn push_front(&mut self, v: V)
        requires old(self).well_formed(),
        ensures self.well_formed(), self@ == seq![v].add(old(self)@),
    {
        match &self.head {
            None => {
                // Special case: list is empty
            ...
            Some(head_ptr) => {
                ...
                let tracked (mut head_perm, head_dealloc) = self.perms.borrow_mut().tracked_remove(0);
                ...
                proof {
                    ...
                    assert forall|j: nat| 0 <= j && j < old(self)@.len() implies self.perms@.dom().contains(j) by {
                        assert(old(self).wf_perm(j));
                    }
                ...
                assert(self.wf_perm(0));
                assert(forall|i: nat| 1 <= i && i <= old(self).ptrs@.len() &&
                    old(self).wf_perm((i - 1) as nat) ==> #[trigger] self.wf_perm(i));

                assert(self@[0] == v);
                assert forall|i: int| 1 <= i && i <= self.ptrs@.len() as int - 1 implies old(self)@[i - 1] == self@[i] by {
...
impl<'a, V> Iterator<'a, V> {
    ...
    pub fn new(l: &'a DoublyLinkedList<V>) -> (it: Self)
        requires l.well_formed(), l@.len() > 0,
        ensures it.valid(), it.index() == 0, it.list() == l,
    {
        Iterator {
            l,
            cur: l.head,
    ...
    pub fn value(&self) -> (v: &V)
        requires self.valid(),
        ensures v == self.list()@[self.index() as int],
    {
