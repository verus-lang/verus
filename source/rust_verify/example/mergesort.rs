...
verus! {
    ...
    pub proof fn lemma_seq_count_value_zero<A>(s: Seq<A>, a: A)
        requires forall|i: nat| i < s.len() ==> s[i as int] != a
        ensures seq_count_value(s, a) == 0
        decreases s.len()
    {
        if s.len() == 0 {
        } else {
    ...
    pub proof fn lemma_seq_drop_index_count_value<A>(s: Seq<A>, i: nat, a: A)
        ...
        ensures
            seq_count_value(s.subrange(0, i as int) + s.subrange((i + 1) as int, s.len() as int), a) == seq_count_value(s, a) - 1
        decreases s.len()
    {
        if i == s.len() - 1 {
            assert(seq_count_value(s.subrange(0, i as int) + s.subrange((i + 1) as int, s.len() as int), a) == seq_count_value(s, a) - 1);
        } else {
            lemma_seq_drop_index_count_value(s.drop_last(), i, a);
            assert(seq_count_value(s.drop_last().subrange(0, i as int) + s.drop_last().subrange((i + 1) as int, s.len() - 1 as int), a) == seq_count_value(s.drop_last(), a) - 1);
            assert(s.drop_last().subrange(0, i as int) == s.subrange(0, i as int));
            assert(s.drop_last().subrange((i + 1) as int, s.len() - 1 as int) == s.subrange((i + 1) as int, s.len() - 1 as int));
            reveal_with_fuel(seq_count_value, 4);
            assert(seq_count_value(s.subrange(0, i as int) + s.subrange((i + 1) as int, s.len() - 1 as int), a) == seq_count_value(s.drop_last(), a) - 1);
            assert(seq_count_value(s.drop_last(), a) - 1 == seq_count_value(s, a) - 1);
            assert((s.subrange(0, i as int) + s.subrange((i + 1) as int, s.len() as int)).last() == s.last());
            assume(seq_count_value(s.subrange(0, i as int) + s.subrange((i + 1) as int, s.len() - 1 as int), a) ==
                seq_count_value(s.subrange(0, i as int) + s.subrange((i + 1) as int, s.len() as int), a)); // TODO
        }
    }
            ...
            s1.len(),
    {
        ...
        if forall|a: A| seq_count_value(s1, a) == 0 {
            ...
            assert(exists|i2: nat| i2 < s2.len() && s2[i2 as int] == a && forall|j: nat| i2 < j < s2.len() ==> s2[j as int] != a) by {
                assert(exists|i2: nat| i2 < s2.len() && s2[i2 as int] == a);
                if forall|i2: nat| (i2 < s2.len() ==> s2[i2 as int] != a) || exists|j: nat| i2 < j < s2.len() ==> s2[j as int] != a {
                    // ... ???
                    assume(false);
                }
            }
            let i2 = choose|i2: nat| i2 < s2.len() && s2[i2 as int] == a && forall|j: nat| i2 < j < s2.len() ==> s2[j as int] != a;
