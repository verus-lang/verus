...
pub(crate) mod parsing {
    ...
    pub(crate) fn ambig_ty(
            ...
            || lookahead.peek(Token![extern])
        {
            ...
            let fn_spec = crate::verus::parse_fn_spec(input)?;
        ...
        pub(crate) fn parse(input: ParseStream, allow_plus: bool) -> Result<Self> {
            if input.peek(Token![->]) {
                ...
                if Self::is_pat_return(&input.fork())? {
                    ...
                    let pat = crate::pat::Pat::parse_single(&content)?;
                    let colon = content.parse()?;
                    let allow_group_generic = true;
                    let ty = ambig_ty(&content, allow_plus, allow_group_generic)?;
                    if content.is_empty() {
    ...
    pub(crate) fn parse_bare_fn_arg(input: ParseStream, allow_self: bool) -> Result<BareFnArg> {
